...
def user_create(client):
    response = client.post(
        "/users/",
        json={"username": "hello", "password": "hello", "email": "hello@gmail.com"},
    )
...
def like_create(user_create, post_create, session):
    like_data = {"user_id": user_create["user_id"], "post_id": post_create[0].post_id}
    session.add(models.Vote(**like_data))
    session.commit()
    likes = (
        session.query(models.Vote)
        .filter(models.Vote.post_id == post_create[0].post_id)
        .filter(models.Vote.user_id == user_create["user_id"])
        .first()
    )
    return likes

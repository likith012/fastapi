...
def test_with_no_user(client):
    response = client.post("/token/", data={"username": "hello@gmail.com", "password": "hello"})
    assert response.status_code == 404
    
    
def test_wrong_credentials(client, user_create):
    wrong_password = "hello123"
    response = client.post("/token/", data={"username": user_create['email'], "password": wrong_password})
    assert response.status_code == 401
    
    
def test_correct_credentials(client, user_create):
    response = client.post("/token/", data={"username": user_create['email'], "password": user_create['password']})
    assert response.status_code == 200 # Not 201    
    

def test_access_token(client, user_create):
    response = client.post("/token/", data={"username": user_create['email'], "password": user_create['password']})
    payload = jwt.decode(response.json()["access_token"], settings.SECRET_KEY, algorithms=[settings.ALGORITHM])

...
@router.get("/", status_code=status.HTTP_200_OK, response_model=List[schemas.ResponseOut])
def get_posts(limit: Union[int, None] = None, skip: int = 0, search: Union[str, None] = None, db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
    ...
    if search is None:
        getall_posts = posts_with_votes.filter(models.Post.owner_id == current_user.user_id)
    else:
        getall_posts = posts_with_votes.filter(models.Post.owner_id == current_user.user_id).filter(models.Post.content.contains(search))
...
def create_post(post: schemas.RequestCreate, db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
